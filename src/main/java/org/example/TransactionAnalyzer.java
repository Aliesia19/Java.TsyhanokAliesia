...
public abstract class TransactionAnalyzer {
    ...
    public static FindTopResult findTopTransactionsByMonth(String monthYear , List<Transaction> transactions,DateTimeFormatter dateFormatter) {
        ...
        result.ExpensiveTransactions = filteredTransactions.stream().sorted(Comparator.comparing(Transaction::getAmount)).limit(10).collect(Collectors.toList());
        result.CheapestTransactions=filteredTransactions.stream().sorted(Comparator.comparing(Transaction::getAmount).reversed())
